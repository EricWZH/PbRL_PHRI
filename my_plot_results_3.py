...
def plot_curve(axs, x, mean_total_return_array, std_total_return_array,
    ...
    axs[0, 0].fill_between(
        ...
        lw=0,
        color=color)
    axs[0, 0].set_ylabel('Episode True Return', fontsize='x-large')
    axs[0, 0].set_xlabel('Environment Steps', fontsize='x-large')
    axs[0, 1].plot(x[index], mean_task_success_array[index], lw=2, color=color)
    axs[0, 1].fill_between(
        ...
        lw=0,
        color=color)
    axs[0, 1].set_ylabel('Success Rate', fontsize='x-large')
    axs[0, 1].set_xlabel('Environment Steps', fontsize='x-large')
    axs[1, 0].plot(x[index], mean_pref_return_array[index], lw=2, color=color)
    axs[1, 0].fill_between(
        ...
        lw=0,
        color=color)
    axs[1, 0].set_ylabel('Episode Preference Return', fontsize='x-large')
    axs[1, 0].set_xlabel('Environment Steps', fontsize='x-large')
    for i in range(3):
        axs[i // 2, i % 2].set_facecolor('white')
        axs[i // 2, i % 2].spines['bottom'].set_color('black')
        axs[i // 2, i % 2].spines['bottom'].set_linewidth(3.0)
        axs[i // 2, i % 2].spines['left'].set_color('black')
        axs[i // 2, i % 2].spines['left'].set_linewidth(3.0)
        axs[i // 2, i % 2].spines['top'].set_color('black')
        axs[i // 2, i % 2].spines['top'].set_linewidth(3.0)
        axs[i // 2, i % 2].spines['right'].set_color('black')
        axs[i // 2, i % 2].spines['right'].set_linewidth(3.0)
        axs[i // 2, i % 2].grid(color='black', axis='y', ls='--')
    axs[1, 1].axis('off')
...
if __name__ == '__main__':
    ...
    if on_policy:
        path_list = [
            'PrefPPO', 'PrefPPO_noisy', 'PrefPPO_mistake', 'PrefPPO_skip',
            'PrefPPO_equal', 'PrefPPO_myopic', 'DecoupledPrefPPO_8',
            'DecoupledPrefPPO_8_noisy', 'DecoupledPrefPPO_8_mistake',
            'DecoupledPrefPPO_8_skip', 'DecoupledPrefPPO_8_equal',
            'DecoupledPrefPPO_8_myopic'
        ]
        labels = [
            'Oracle PrefPPO', 'Noisy PrefPPO', 'Mistake PrefPPO',
            'Skip PrefPPO', 'Equal PrefPPO', 'Myopic PrefPPO',
            'Oracle CG-PrefPPO', 'Noisy CG-PrefPPO', 'Mistake CG-PrefPPO',
            'Skip CG-PrefPPO', 'Equal CG-PrefPPO', 'Myopic CG-PrefPPO'
        ]
        ...
        n_steps = 200
        color = [
            'indigo', 'mediumseagreen', 'gold', 'aqua', 'lightslategray',
            'midnightblue', 'red', 'violet', 'darkcyan', 'brown',
            'darkslategrey', 'chocolate'
        ]
        ...
        fig.legend(fontsize='x-large',
                   loc='upper left',
                   ncol=2,
                   bbox_to_anchor=(0.505, 0.42))
        plt.tight_layout()
        plt.savefig('on_policy.png')
        plt.show()
    else:
        path_list = [
            'PEBBLE', 'PEBBLE_noisy', 'PEBBLE_mistake', 'PEBBLE_skip',
            'PEBBLE_equal', 'PEBBLE_myopic', 'DecoupledPEBBLE_8',
            'DecoupledPEBBLE_8_noisy', 'DecoupledPEBBLE_8_mistake',
            'DecoupledPEBBLE_8_skip', 'DecoupledPEBBLE_8_equal',
            'DecoupledPEBBLE_8_myopic'
        ]
        labels = [
            'Oracle PEBBLE', 'Noisy PEBBLE', 'Mistake PEBBLE', 'Skip PEBBLE',
            'Equal PEBBLE', 'Myopic PEBBLE', 'Oracle CG-PEBBLE',
            'Noisy CG-PEBBLE', 'Mistake CG-PEBBLE', 'Skip CG-PEBBLE',
            'Equal CG-PEBBLE', 'Myopic CG-PEBBLE'
        ]
