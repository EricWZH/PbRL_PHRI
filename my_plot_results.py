...
def read_csv_data(par_path, tensorboard_log_list, total_timesteps, n_envs, n_steps, smooth):
...
def plot_curve(axs, x, mean_total_return_array, std_total_return_array, mean_pref_return_array, std_pref_return_array,
               mean_task_success_array, std_task_success_array, color, label):
    index = np.arange(0, len(x), len(x) // 100)
    axs[0].plot(x[index], mean_total_return_array[index], lw=2, color=color, label=label)
    axs[0].fill_between(x[index],
    ...
    axs[1].plot(x[index], mean_task_success_array[index], lw=2, color=color)
    axs[1].fill_between(x[index],
...
if __name__ == '__main__':
    ...
    if on_policy:
        path_list = [
            'PPO', 'PrefPPO/teacher_-1_1.0_0.0_0.0_0.0', 'DecoupledPrefPPO_init', 'DecoupledPrefPPO_16'
        ]
        labels = ['PPO', 'PrefPPO', 'DecoupledPrefPPO', 'PrefPPO_CL']
        ...
        for i in range(len(path_list)):
            ...
            x, mean_total_return_array, std_total_return_array, mean_pref_return_array, std_pref_return_array, mean_task_success_array, std_task_success_array = read_csv_data(
                path_list[i], file_path_list, total_timesteps, n_envs, n_steps, smooth=5)
            plot_curve(axs, x, mean_total_return_array, std_total_return_array, mean_pref_return_array,
                       std_pref_return_array, mean_task_success_array, std_task_success_array, color[i], labels[i])

        fig.legend(fontsize='x-large', loc='upper left', bbox_to_anchor=(0.77, 0.37))
        plt.savefig('on_policy.png')
        plt.show()
    else:
        path_list = [
            'logs/SAC', 'logs/PEBBLE/teacher_-1_1.0_0.0_0.0_0.0', 'logs/DecoupledPEBBLE/teacher_-1_1.0_0.0_0.0_0.0'
        ]
        labels = ['SAC', 'PEBBLE', 'DecoupledPEBBLE']
        ...
        for i in range(len(path_list)):
            file_path_list = os.listdir(path_list[i])
            x, mean_total_return_array, std_total_return_array, mean_pref_return_array, std_pref_return_array, mean_task_success_array, std_task_success_array = read_csv_data(
                path_list[i], file_path_list, total_timesteps, n_envs, n_steps, smooth=100)
            plot_curve(axs, x, mean_total_return_array, std_total_return_array, mean_pref_return_array,
                       std_pref_return_array, mean_task_success_array, std_task_success_array, color[i], labels[i])

        fig.legend(fontsize='x-large', loc='upper left', bbox_to_anchor=(0.77, 0.37))
        plt.savefig('off_policy.pdf')
        plt.show()