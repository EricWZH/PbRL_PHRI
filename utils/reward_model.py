            ...
            if full_start < len(obs):
                ...
                for idx in range(len(full_obs) // batch_size + 1):
                    ...
                    if start < len(full_obs):
                        ...
                        dist = torch.norm(obs[full_start:full_end, None, :].to(device) -
                                          full_obs[None, start:end, :].to(device),
                                          dim=-1,
                                          p=2)
                        dists.append(dist)
...
class RewardModel:
    def __init__(self,
        ...
        self._current_progress_remaining = _current_progress_remaining
    ...
    def construct_ensemble(self):
        for i in range(self.de):
            model = nn.Sequential(
                *gen_net(in_size=self.ds +
                         self.da, out_size=1, H=256, n_layers=3, activation=self.activation)).float().to(device)
    ...
    def get_queries(self, mb_size=20):
        # get mb_size trajectory pairs from self.inputs and self.targets
        ...
        if self.size_segment < len_traj:
            ...
            # Generate time index
            time_index = np.array(
                [list(range(i * len_traj, i * len_traj + self.size_segment)) for i in range(mb_size)])
            time_index_2 = time_index + np.random.choice(len_traj - self.size_segment, size=mb_size,
                                                         replace=True).reshape(-1, 1)
            time_index_1 = time_index + np.random.choice(len_traj - self.size_segment, size=mb_size,
                                                         replace=True).reshape(-1, 1)
