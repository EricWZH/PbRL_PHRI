...
def read_csv_data(par_path, tensorboard_log_list, total_timesteps, n_envs,
    ...
    std_total_return_array = np.std(smooth_total_return_array, axis=0) / 4
    mean_pref_return_array = np.mean(smooth_pref_return_array, axis=0)
    std_pref_return_array = np.std(smooth_pref_return_array, axis=0) / 4
    mean_task_success_array = np.mean(smooth_task_success_array, axis=0)
    std_task_success_array = np.std(smooth_task_success_array, axis=0) / 4
...
if __name__ == '__main__':
    ...
    if on_policy:
        #'''
        path_list = [
            'DecoupledPrefPPO_2', 'DecoupledPrefPPO_4', 'DecoupledPrefPPO_1',
            'DecoupledPrefPPO_3'
        ]
        labels = [
            'based on distance (all)', 'based on distance (middle)',
            'based on distance (head)', 'based on distance (tail)'
        ]
        '''
        path_list = [
            'DecoupledPrefPPO_8', 'DecoupledPrefPPO_9', 'DecoupledPrefPPO_10',
            'DecoupledPrefPPO_11'
        ]
        labels = [
            'based on grade (all)', 'based on grade (middle)',
            'based on grade (head)', 'based on grade (tail)'
        ]
        '''
        ...
        n_envs = 16
        n_steps = 200
        color = ['red', 'seagreen', 'sienna', 'royalblue']
        ...
        '''
        path_list = [
            'DecoupledPEBBLE_1', 'DecoupledPEBBLE_3', 'DecoupledPEBBLE_2',
            'DecoupledPEBBLE_4'
        ]
        labels = [
            'based on distance (all)', 'based on distance (middle)',
        ...
        '''
        path_list = [
            'DecoupledPEBBLE_8', 'DecoupledPEBBLE_9', 'DecoupledPEBBLE_10',
            'DecoupledPEBBLE_11'
        ]
        labels = [
            'based on grade (all)', 'based on grade (middle)',
